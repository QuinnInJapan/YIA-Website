  ...
  return (
    <Link href={`/blog/${post.slug}`} className="blog-card">
      <div className="blog-card__image-wrap">
        {heroSrc ? (
          <LazyImage src={heroSrc} alt={ja(post.title)} />
        ) : (
      ...
      <div className="blog-card__body">
        <div className="blog-card__meta">
          {ja(post.category) && <span className="blog-card__category">{ja(post.category)}</span>}
        ...
        {ja(post.excerpt) && <p className="blog-card__excerpt">{ja(post.excerpt)}</p>}
      </div>
    </Link>
  );
}
